...
final_model <- mod7
dim(test_new)
predictions <- predict(final_model,test_new)

test_new$PredictedChurn <- predictions
res.con <- condes(test_new, num.var = 24)
res.con$quanti
res.con$quali
...
#                      RSQRT = round(c(RSQUARE(exp(fitted(final_model)), train_new$Churn),
#                               RSQUARE(exp(predict(final_model, newdata = test_new)), test_new$Churn)),2),
#                      RMSE = round(c(rmse(exp(fitted(final_model)), train_new$Churn),
#                               rmse(exp(predict(final_model, newdata = test_new)), test_new$Churn)),2));results
...
# index <- createDataPartition(df2$Churn, p = 0.7, list = FALSE)
# # Create training and testing datasets
# train_new <- df2[index, ]
# test_new <- df2[-index, ]
# 
# final_model2 <- mod7 <- glm(Churn ~ SeniorCitizen +  poly(tenure, 2) + MultipleLines + OnlineSecurity*TechSupport + OnlineBackup + MonthlyCharges + Contract + PaperlessBilling, data=train_new, family = binomial)
# # summary(final_model2)
# #Validate
# predictions <- predict(final_model2, newdata = test_new)
# actual_values <- test_new$Churn
# 
# rmse <- sqrt(mean((predictions - actual_values)^2))
# r_squared <- 1 - sum((actual_values - predictions)^2) / sum((actual_values - mean(actual_values))^2)
# 
# cat("Root Mean Squared Error (RMSE):", rmse, "\n")
# cv_response_variable <- sd(test_new$Churn) / mean(test_new$Churn);cv_response_variable
# cv_rmse_ratio <- rmse / mean(test_new$Churn);cv_rmse_ratio
# 
...
# observations_with_large_residuals <- test_new[large_residual_indices, ]
